...
def load_carbon_data():
    carbonSequesteredDf = pd.read_csv(
        "static/data/TheGreatCarbonSinkInfo-Carbon-Rate.csv"
    )
...
if age != 0:
    ...
    x = np.arange(60)
    y = (
        carbon_sequestered_df[tree].values * 0.00045359237
    )  # all values are being converted to tons
    ...
    if round(co2_total, 3) < 0.001:
        execute_markdown(
            f"<li>Cutting down this tree would lead to < 0.001 tons of CO2 being released </li>"
        )
    else:
        execute_markdown(
            f"<li>Cutting down this tree would lead to {round(co2_total,3)} tons of CO2 being released </li>"
        )
    execute_markdown(
        f"<li>To off-set this you would need to plant {trees_needed} tree(s), costing £{round(cost_of_trees,2)}</li>"
    )
    execute_markdown(
        f"<li>Or to capture the carbon manually would cost approx £{round(cost_of_fill,2)}</li>"
    )
...
for tree in carbon_sequestered_df.columns[1:]:
    ...
    fig.add_trace(go.Scatter(x=x, y=y, name=tree))
...
sequestered_data = pd.DataFrame(
    {"Tree": total_sequestered.keys(), "Total Sequestered": total_sequestered_values}
)
...
carbon_average = pd.DataFrame(
    {
        "Consumption": carbon_metrics.keys(),
        "Carbon Emissions (tonne)": carbon_metrics.values(),
    }
)
fig = px.bar(carbon_average, x="Consumption", y="Carbon Emissions (tonne)")
fig.update_layout(
    title="Yearly UK Average Household Carbon Emissions", plot_bgcolor="rgba(0,0,0,0)",
)
fig.update_xaxes(showline=True, linewidth=2, linecolor="#B4B1B0", gridcolor="#B4B1B0")
fig.update_yaxes(showline=True, linewidth=2, linecolor="#B4B1B0", gridcolor="#B4B1B0")
st.plotly_chart(fig, use_container_width=True)
...
execute_markdown(
    '<iframe src="https://ourworldindata.org/grapher/co-emissions-per-capita?tab=chart&stackMode=absolute&region=World" loading="lazy" style="width: 100%; height: 600px; border: 0px none;"></iframe>'
)
...
# Sponsorship logo
st.image(
    "https://thelandtrust.org.uk/wp-content/uploads/2015/09/Woodland-Trust.png",
    use_column_width=True,
)
